...
public final class DestinationProperty implements ICartProperty<String> {
    ...
    private void commandSetProperty(
        ...
        commandSetProperty(sender, properties, destination, null, null);
    }
    
    @CommandTargetTrain
    @PropertyCheckPermission("destination")
    @CommandMethod("train destination|dest <destination> <destination1> <destination2>")
    @CommandDescription("Sets a new destination for the train to go to")
    private void commandSetProperty(
            final CommandSender sender,
            final TrainProperties properties,
            @Quoted @Argument(value="destination", suggestions="destinations") String destination,
            final @Quoted @Argument(value="destination1", suggestions="destinations") String destination1,
            final @Quoted @Argument(value="destination2", suggestions="destinations") String destination2
    ) {
    	if(destination1 != null) destination = destination + " " + destination1;
    	if(destination2 != null) destination = destination + " " + destination2;
