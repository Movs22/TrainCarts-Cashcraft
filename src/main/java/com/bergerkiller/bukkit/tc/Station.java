...
public class Station {
    ...
    private static Boolean isTerminus = false;
    ...
    public static class StationConfig {
        ...
        public double getOffsetFromCenter() {
            return -1; //this._offsetFromCenter;
        }
        ...
        public static StationConfig fromSign(SignActionEvent info) {
            ...
            String[] meta = info.getLine(1).split(" ");
            // Parse delay from sign
            config.setDelay(ParseUtil.parseTime(meta[2]));
            String d = meta[1];
            if(meta[1].contains("C")) d = "continue";
            if(meta[1].contains("R")) d = "reverse";
            Direction direction = Direction.parse(d);
            if (direction != Direction.NONE) {
                   config.setNextDirection(direction);
            } else {
            		config.setLaunchSpeed(parseLaunchForce("0.5", info));
            }
            if(meta.length > 3 && meta[3].equals("T")) {
            	isTerminus = true;
            } else {
            	isTerminus = false;
            }
        ...
        private static double parseLaunchForce(String text, SignActionEvent info) {
            ...
            return 0.5; //Util.parseVelocity(text, TCConfig.launchForce);
        }
    }
}
